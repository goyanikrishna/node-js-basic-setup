...
exports.account = {
        ...
        try {
            ...
            if (userInfo) {
                ...
                if (!userInfo.isActive) {
                    return badRequestResponse(res, {
                        message: "Your account is deactivated, please activate your account from here",
    ...
    forgetPassword: async function(req, res) {
        try {
            ...
            if (emailSended.accepted) {
                await USER.findOneAndUpdate({ _id: userInfo._id }, {
                    $set: {
                ...
                return successResponse(res, {
                    message: "Forget password otp code send, please check your email account.",
                });
            } else {
                return badRequestResponse(res, {
    ...
    changePassword: async function(req, res) {
        try {
            ...
            userInfo.password = req.body.newPassword;
            var isUpdated = await USER.findOneAndUpdate({ _id: userInfo._id }, {
                $set: {
                    password: userInfo.password,
                },
            });
    ...
    activateAccount: async function(req, res) {
        try {
            ...
            if (userInfo.accountActivationCode == req.body.accountActivationCode) {
                const isUpdated = await USER.findOneAndUpdate({ _id: userInfo._id }, {
                    $set: {
                        isActive: true,
                    },
                });
